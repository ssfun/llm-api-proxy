...
export default async function handler(request: Request): Promise<Response> {
  ...
  logInfo("Background 处理请求", { reqId, method: request.method, path: userPath, service });

  const upstreamURL = buildUpstreamURL(proxy.host, proxy.basePath, userPath, originalSearch || requestUrl.search);
  ...
  try {
    const response = await fetchWithRetry(
      upstreamURL,
      { method: request.method, headers: forwardHeaders, body: request.method === "GET" || request.method === "HEAD" ? undefined : request.body },
      {
        retries,
    ...
    return new Response(buffer, { status: response.status, statusText: response.statusText, headers: processedHeaders });
  } catch (err) {
